...
- **🐴 Sleek Horse Head Bar Widget**: Sits cleanly on the top-right status bar with calibrated optical alignment and theme integration. Smoothly transitions into an animated spinner (`󰑋`) with active accent colors and progress tooltips during live downloads.
- **🐎 Animated Galloping Pony Progress Tracker**: Features a crisp pixel-art galloping pony sprite (`caballoNormal.png`), running at full stride in the modal header badge and smoothly galloping across the download card tracking live percentage ticks in real-time!
